...
- Open: `sql.Open("sqlite3", "file:<path>?mode=ro&_query_only=1&_foreign_keys=on&_busy_timeout=5000")`. `immutable=1` is added when no WAL/SHM/journal sidecar files exist next to the DB (truly read-only filesystem); otherwise the reader uses pure `mode=ro` so the upstream writer's WAL commits remain visible.
...
| `wacli-reader messages list [filters]` | List messages. Filters: `--chat`, `--sender`, `--from-me`/`--from-them`, `--asc`, `--limit`, `--after`, `--before`, `--forwarded`, `--starred`. |
| `wacli-reader messages search <query> [filters]` | FTS5 (or `LIKE`) message search. Filters: `--chat`, `--from`, `--has-media`, `--type`, `--limit`, `--after`, `--before`, `--forwarded`, `--starred`. |
| `wacli-reader messages starred [filters]` | List starred messages by stored star time. Filters: `--chat`, `--limit`, `--after`, `--before`, `--asc`. |
...
| `wacli-reader messages export [--chat JID] [--limit N] [--after DATE] [--before DATE] [--output PATH]` | Export messages as JSON, oldest first. |
| `wacli-reader calls list [filters]` | List call events. Filters: `--chat`, `--limit`, `--after`, `--before`, `--asc`. |
| `wacli-reader polls list [--chat JID] [--limit N]` | List stored polls. |
| `wacli-reader poll show --chat JID --id MSG_ID` | Show one poll's question, options, and votes. |
| `wacli-reader store stats` | Row counts (chats, groups, left groups, messages). |
